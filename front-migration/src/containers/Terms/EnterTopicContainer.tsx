...
export default function EnterTopicContainer() {
  ...
  const { enterTopicTerms, enterTopicTermsNext, newsEnterTopicLoading } =
  ...
  useEffect(() => {
    ...
    if (enterTopicTerms) {
      setTerms(enterTopicTerms.data.slice(0, 10));
    }
    if (enterTopicTermsNext) {
      setTermsNext(enterTopicTermsNext.data.slice(0, 10));
    }
    setIsReady(true);
    isEnd.current = false;
  }, [enterTopicTerms, enterTopicTermsNext]);
